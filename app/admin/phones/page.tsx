...
export default function PhonesPage() {
  ...
  // Phones admin: only mobile devices (type=device). Accessories live in /admin/accessories.
  const { data: products, loading, error, clearError, create, update, remove, bulkRemove, pagination, setPage } = useAdminApi<Product>({ endpoint: "/api/admin/products?types=device", paginate: { limit: 100 } });
  ...
  return (
    <div>
      <PageHeader title="📱 الهواتف" count={pagination?.total ?? products.length} onAdd={openCreate} addLabel="هاتف جديد" />
      ...
      {/* Products list */}
      {filtered.length === 0 ? (
        <EmptyState icon="📱" title="لا توجد هواتف" sub="أضف أول هاتف" />
      ) : (
