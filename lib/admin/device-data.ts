// =====================================================
// ClalMobile — Unified Device Data Fetcher
// Consolidates gsmarena.ts and mobileapi.ts behind a single API
// Default: mobileapi (fast API). Fallback: gsmarena (scraper).
// Combined: mobileapi data enriched with gsmarena images.
// =====================================================

import { fetchProductData as fetchFromMobileAPI } from "./mobileapi";
import { fetchProductData as fetchFromGSMArena } from "./gsmarena";

export type DeviceProvider = "mobileapi" | "gsmarena" | "combined";

// Re-export for direct access when needed
export { fetchFromMobileAPI, fetchFromGSMArena };
...
/**
 * Fetch device data from the specified provider.
 * Default: mobileapi (fast API). Fallback: gsmarena (scraper).
 * Combined: mobileapi data enriched with gsmarena images.
 */
export async function fetchDeviceData(
  name: string,
  brand: string,
  provider: DeviceProvider = "mobileapi"
) {
  switch (provider) {
    case "combined":
      return fetchCombined(name, brand);
    case "gsmarena":
      return fetchFromGSMArena(name, brand);
    case "mobileapi":
    default:
      return fetchFromMobileAPI(name, brand);
  }
}
